...
export function findImports(filePath: string) {
  ...
  // match string which contain an import
  const reOneLineString = new RegExp(`["'\`].*(${_reImport}).*["'\`]`, "g");
  // match multi lines string which contain an import
  ...
  while ((matches = reImport.exec(fileContent)) !== null) {
    ...
    // @note This is necessary to avoid infinite loops with zero-width matches.
